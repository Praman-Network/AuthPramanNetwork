...
export function SecurityDashboard({ email, embeddedWalletAddress }: SecurityDashboardProps) {
  const { linkWallet, linkEmailWallet, isLinking, error, linkedAddress } = useAccountLinking(embeddedWalletAddress);
  const { login } = (window as any).privy || {};

  const isMetaMaskSession = !email;

  const handleLinkEmail = async () => {
    if (!login) {
      console.error("Privy login hook is not available");
      return;
    }
    // For demo purposes, we pass dummy hooks to linkEmailWallet
    // In reality, privy hooks should be used within a PrivyProvider context
    await linkEmailWallet(
      async () => { /* open privy modal */ },
      async () => { return { getEthereumProvider: () => (window as any).ethereum } }
    );
  };
  ...
  return (
    <div className="bg-zinc-950/80 border border-zinc-800/80 rounded-2xl p-6 shadow-xl max-w-md mx-auto relative overflow-hidden text-zinc-100">
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <p className="text-xs text-zinc-400 uppercase tracking-wider font-bold mb-1">Active Wallet (SBT Holder)</p>
      ...
      {!linkedAddress && (
        <button
          onClick={isMetaMaskSession ? handleLinkEmail : linkWallet}
          disabled={isLinking}
        ...
        >
          {isLinking ? (
            ...
            isMetaMaskSession ? 'Link Email Wallet (Privy)' : 'Link External Web3 Wallet'
          )}
        </button>
      )}
    </div>
